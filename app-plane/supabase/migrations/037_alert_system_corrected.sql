-- Custom Alert Generation System Migration (Corrected)
-- Implements user preferences, component watching, and alert delivery tracking
-- Fixed for: gen_random_uuid(), tenants table reference, missing components table
-- Created: 2025-11-19
...
-- ============================================================================
-- 3. Component Watch Table (Placeholder - components table doesn't exist yet)
-- ============================================================================

-- Note: This table will work once components table is created
CREATE TABLE IF NOT EXISTS component_watches (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  component_id UUID NOT NULL,  -- No FK constraint yet, components table doesn't exist
...
-- ============================================================================
-- 5. Alert Generation Functions (Simplified - components table doesn't exist)
-- ============================================================================
...
SELECT
  '✅ Alert system tables created' as status,
  string_agg(table_name, ', ' ORDER BY table_name) as tables
FROM information_schema.tables
WHERE table_schema = 'public'
  AND table_name IN ('alert_preferences', 'component_watches', 'alert_deliveries');
