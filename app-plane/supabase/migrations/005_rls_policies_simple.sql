-- Row Level Security Policies - Simplified for Standalone PostgreSQL
...
CREATE POLICY "Users can view BOM line items in their organization"
  ...
  USING (
    EXISTS (
      ...
      AND boms.organization_id = public.current_user_organization_id()
    )
  );
...
-- =====================================================
-- BYPASS POLICIES FOR SUPERUSER (for testing/admin)
-- =====================================================

-- Allow postgres superuser to bypass RLS for admin tasks
ALTER TABLE organizations FORCE ROW LEVEL SECURITY;
ALTER TABLE user_profiles FORCE ROW LEVEL SECURITY;
ALTER TABLE components FORCE ROW LEVEL SECURITY;
ALTER TABLE boms FORCE ROW LEVEL SECURITY;
ALTER TABLE bom_line_items FORCE ROW LEVEL SECURITY;
ALTER TABLE alerts FORCE ROW LEVEL SECURITY;

-- Create admin bypass policies
CREATE POLICY "Superuser can do anything on organizations"
  ON organizations FOR ALL
  USING (current_user = 'postgres' OR current_user = 'supabase_admin')
  WITH CHECK (current_user = 'postgres' OR current_user = 'supabase_admin');

CREATE POLICY "Superuser can do anything on user_profiles"
  ON user_profiles FOR ALL
  USING (current_user = 'postgres' OR current_user = 'supabase_admin')
  WITH CHECK (current_user = 'postgres' OR current_user = 'supabase_admin');

CREATE POLICY "Superuser can do anything on components"
  ON components FOR ALL
  USING (current_user = 'postgres' OR current_user = 'supabase_admin')
  WITH CHECK (current_user = 'postgres' OR current_user = 'supabase_admin');

CREATE POLICY "Superuser can do anything on boms"
  ON boms FOR ALL
  USING (current_user = 'postgres' OR current_user = 'supabase_admin')
  WITH CHECK (current_user = 'postgres' OR current_user = 'supabase_admin');

CREATE POLICY "Superuser can do anything on bom_line_items"
  ON bom_line_items FOR ALL
  USING (current_user = 'postgres' OR current_user = 'supabase_admin')
  WITH CHECK (current_user = 'postgres' OR current_user = 'supabase_admin');

CREATE POLICY "Superuser can do anything on alerts"
  ON alerts FOR ALL
  USING (current_user = 'postgres' OR current_user = 'supabase_admin')
  WITH CHECK (current_user = 'postgres' OR current_user = 'supabase_admin');
...
SELECT
  ...
  (SELECT COUNT(*) FROM pg_policies WHERE schemaname = 'public' AND tablename = 'bom_line_items') as bom_line_policies,
