...
interface AlertFieldProps {
  source?: string;
  // Accept label prop for react-admin field compatibility (unused here)
  label?: string;
}
 ...
 */
const AlertSeverityField: React.FC<AlertFieldProps> = ({ source = 'severity' }) => {
 ...
 */
const AlertTypeField: React.FC<AlertFieldProps> = ({ source = 'alert_type' }) => {
  const record = useRecordContext();
  if (!record) return null;
