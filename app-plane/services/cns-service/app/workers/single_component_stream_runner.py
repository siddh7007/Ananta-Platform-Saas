"""
Single Component Stream Consumer Runner Module

Provides start/stop functions for the SingleComponentStreamConsumer
to be called from main.py lifespan events.

This consumer listens to RabbitMQ Streams for component enrichment events
(component.enrich.request, component.enrich.force, component.enrich.batch) and
starts the SingleComponentEnrichmentWorkflow in Temporal.
...
Usage in main.py:
    ...
    # In lifespan startup:
    component_stream_task = start_single_component_stream_consumer()
...
from app.workers.single_component_stream_consumer import SingleComponentStreamConsumer

logger = logging.getLogger(__name__)

# Global task reference
_consumer_task: Optional[asyncio.Task] = None
_consumer_instance: Optional[SingleComponentStreamConsumer] = None


def start_single_component_stream_consumer() -> Optional[asyncio.Task]:
    """
    Start SingleComponentStreamConsumer as a background task.
    ...
    if _consumer_task is not None:
        logger.warning("[COMPONENT-STREAM] Single component stream consumer already running")
        return _consumer_task

    try:
        # Create consumer instance
        _consumer_instance = SingleComponentStreamConsumer()

        # Start consumer in background task
        loop = asyncio.get_event_loop()
        _consumer_task = loop.create_task(_run_consumer())

        logger.info("[COMPONENT-STREAM] Single component stream consumer started as background task")
        return _consumer_task
...
async def _run_consumer():
    ...
    try:
        ...
        logger.info("[COMPONENT-STREAM] Consumer cancelled (shutdown requested)")
        raise  # Re-raise to allow task cancellation to complete

    except Exception as e:
        logger.error(f"[FAIL] Single component stream consumer error: {e}", exc_info=True)

    finally:
        logger.info("[COMPONENT-STREAM] Consumer task ended")


async def stop_single_component_stream_consumer() -> None:
    """
    Stop the SingleComponentStreamConsumer background task.
    ...
    if _consumer_task is None:
        logger.debug("[COMPONENT-STREAM] No consumer task to stop")
        return

    try:
        logger.info("[COMPONENT-STREAM] Stopping single component stream consumer...")
        ...
        logger.info("[OK] Single component stream consumer stopped")

    except Exception as e:
        logger.error(f"[FAIL] Error stopping component stream consumer: {e}", exc_info=True)
...
def is_consumer_running() -> bool:
    """
    Check if the single component stream consumer is currently running.
