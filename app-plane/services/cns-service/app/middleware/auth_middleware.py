...
PUBLIC_PATHS: Set[str] = {
    ...
    "/openapi.json",
    "/api/health",
    "/health/",  # Health check endpoints (e.g., /health/config, /health/critical-fixes)
    "/api/docs",
    "/api/redoc",
    ...
    "/api/catalog/search",       # Component catalog search (public, read-only)
    "/api/catalog/browse",       # Component catalog browse (public, read-only)
}

# Path prefixes that don't require authentication
# NOTE: /api/admin/ is INTENTIONALLY NOT listed - admin routes MUST require auth
# SECURITY: /api/catalog/component/ REMOVED - exposes pricing, supplier data, ECCN codes
#           Component lookup now requires authentication
...
QUERY_PARAM_AUTH_PREFIXES: List[str] = [
    ...
    "/api/bom/workflow/",       # BOM workflow SSE endpoints (processing-stream)
]
...
TENANT_ID_HEADER = "X-Tenant-Id"  # Alternative header (same as org_id in our architecture)
...
def extract_actor_from_headers(request: Request) -> tuple[Optional[str], Optional[str]]:
    ...
    # Check both X-Organization-ID and X-Tenant-Id (same entity, different names)
    org_id = request.headers.get(ORG_ID_HEADER) or request.headers.get(TENANT_ID_HEADER)
...
async def _fetch_jwks(domain: str) -> Optional[dict]:
    """
    Fetch JWKS from Auth0/Keycloak domain with caching.

    Args:
        domain: Auth0/Keycloak domain (e.g., your-tenant.us.auth0.com or localhost:8080/realms/ananta-saas)
    ...
    # Fetch fresh JWKS
    # Keycloak uses /protocol/openid-connect/certs, Auth0 uses /.well-known/jwks.json
    # Try Keycloak path first (if domain contains /realms/), then Auth0 path
    if "/realms/" in domain:
        jwks_url = f"http://{domain}/protocol/openid-connect/certs"
    else:
...
async def validate_auth0_token(token: str) -> Optional[dict]:
    ...
    try:
        ...
        # Verify issuer - for Keycloak, accept both http and https, with or without trailing slash
        # Domain format: arc-saas-keycloak:8080/realms/ananta-saas or localhost:8180/realms/ananta-saas
        token_issuer = payload.get("iss", "")

        # Extract realm path from domain (everything after the host:port)
        # e.g., "arc-saas-keycloak:8080/realms/ananta-saas" -> "/realms/ananta-saas"
        domain_parts = auth0_domain.split("/", 1)
        realm_path = "/" + domain_parts[1] if len(domain_parts) > 1 else ""

        # Check if token issuer ends with the realm path (ignoring protocol and host)
        # This allows tokens from localhost:8180 or arc-saas-keycloak:8080
        if not (token_issuer.endswith(realm_path) or token_issuer.endswith(realm_path + "/")):
            logger.warning(
                f"[AuthMiddleware] Auth0 issuer mismatch: "
                f"token={token_issuer} expected_realm={realm_path}"
            )
            return None

        logger.debug(f"[AuthMiddleware] Issuer validated: {token_issuer}")

        # Verify audience (if configured and present in token)
        if auth0_audience:
            aud = payload.get("aud")
            if aud is not None:  # Only validate if token has an audience claim
                # Parse allowed audiences from config (comma-separated)
                allowed_audiences = [a.strip() for a in auth0_audience.split(",")]

                # aud can be string or array
                if isinstance(aud, list):
                    # Token has multiple audiences - check if any match our allowed list
                    if not any(a in allowed_audiences for a in aud):
                        logger.warning(f"[AuthMiddleware] Auth0 audience mismatch: token={aud}, allowed={allowed_audiences}")
                        return None
                elif aud not in allowed_audiences:
                    # Token has single audience - check if it's in our allowed list
                    logger.warning(f"[AuthMiddleware] Auth0 audience mismatch: token={aud}, allowed={allowed_audiences}")
                    return None

                logger.debug(f"[AuthMiddleware] Audience validated: {aud}")
            else:
                # Keycloak admin-cli and some other clients don't set audience
                logger.debug(f"[AuthMiddleware] Token has no audience claim, skipping audience validation")
...
async def validate_api_key(api_key: str) -> Optional[dict]:
    ...
    Supports:
    1. CNS Admin API token (from VITE_CNS_ADMIN_TOKEN / CNS_ADMIN_API_TOKEN env var)
    2. Future: Database-stored API keys

    SECURITY:
    - Uses constant-time comparison to prevent timing attacks
    - Rejects if admin token not configured (fail-secure)
    - Default token only allowed if ALLOW_DEV_DEFAULTS=true (explicit opt-in)
    ...
    # Check against CNS Admin API token (for dashboard/internal tools)
    admin_token = os.getenv("CNS_ADMIN_API_TOKEN") or os.getenv("VITE_CNS_ADMIN_TOKEN")

    # SECURITY: Default token only allowed with explicit opt-in (local dev only)
    if not admin_token:
        allow_dev_defaults = os.getenv("ALLOW_DEV_DEFAULTS", "").lower() in ("true", "1", "yes")
        if allow_dev_defaults:
            admin_token = "f3ab7e1b4c9d2e7f1a3b5c7d9e0f1234567890abcdef1234567890abcdef"
            logger.warning(
                "[AuthMiddleware] Using default dev token - "
                "ALLOW_DEV_DEFAULTS=true (NOT FOR PRODUCTION)"
            )
        else:
            logger.warning(
                "[AuthMiddleware] CNS_ADMIN_API_TOKEN not configured and "
                "ALLOW_DEV_DEFAULTS not enabled - API key auth disabled"
            )
            return None

    # SECURITY: Use constant-time comparison to prevent timing attacks
    if secrets.compare_digest(api_key, admin_token):
        logger.info("[AuthMiddleware] API key validated: CNS Admin Token")
        # Return admin claims - full platform access
        return {
            "sub": "cns-admin-service",
            "email": "admin@cns.internal",
            "role": "admin",
            "organization_id": "platform-admin",
            "permissions": ["*"],  # Full access
            "is_service_account": True
        }

    # TODO: Add database API key validation here
    logger.debug("[AuthMiddleware] API key not recognized")
    return None
    ...
    try:
        ...
        try:
            ...
            if existing:
                ...
                # If user exists but has no organization_id, look up from organization_memberships
                # Use the test org (a0000000...) if user has multiple memberships
                if not org_id:
                    membership = session.execute(
                        text("""
                            ...
                            ORDER BY
                                CASE WHEN om.organization_id = 'a0000000-0000-0000-0000-000000000000'::uuid THEN 0 ELSE 1 END,
                                om.created_at ASC
                            LIMIT 1
                        """),
                        {"user_id": existing.id}
                    ...
                    if membership:
                        ...
                        logger.info(f"[AutoProvision] Found org from membership: {org_id}")

                # Return org_id and empty string (auth0_org_id not stored in DB)
            ...
            org_result = session.execute(
                text("""
                    INSERT INTO organizations (name, slug, max_users, created_at, updated_at)
            ...
            # Create user
            # Note: full_name is a GENERATED column (first_name || ' ' || last_name)
            # So we must insert first_name and last_name separately
            ...
            user_result = session.execute(
                text("""
                    INSERT INTO users (email, first_name, last_name, auth0_user_id, organization_id, created_at)
                    VALUES (:email, :first_name, :last_name, :auth0_id, :org_id, NOW())
                    RETURNING id
                """),
...
async def build_auth_context_from_token(claims: dict, provider: str) -> AuthContext:
    ...
    if provider == "supabase":
        ...
        # If no custom /role claim, check for platform:* roles or realm roles
        if not role:
            auth0_roles = claims.get(f"{namespace}/roles", [])
            # Also check realm_access.roles for Keycloak tokens
            ...
            if isinstance(auth0_roles, list):
                all_roles.extend(auth0_roles)
            if isinstance(realm_roles, list):
                all_roles.extend(realm_roles)

            if all_roles:
                # Map roles to system roles (supports both platform:* and direct role names)
                # Priority order: super_admin > owner > admin > engineer > analyst
                if any(r in ['platform:super_admin', 'super_admin', 'super-admin', 'realm-admin'] for r in all_roles):
                    role = Role.SUPER_ADMIN.value
                elif any(r in ['owner', 'org-owner', 'organization-owner', 'billing_admin'] for r in all_roles):
                    role = Role.OWNER.value
                elif any(r in ['platform:admin', 'admin', 'platform:staff', 'tenant-admin', 'org_admin', 'org-admin'] for r in all_roles):
                    role = Role.ADMIN.value
                elif any(r in ['platform:engineer', 'engineer', 'developer', 'staff'] for r in all_roles):
                    role = Role.ENGINEER.value
                elif any(r in ['analyst', 'user', 'viewer', 'member'] for r in all_roles):
                    role = Role.ANALYST.value
                else:
                    # No recognized role found - default to analyst
                    role = Role.ANALYST.value
                logger.info(f"[AuthMiddleware] Derived role from token roles: {all_roles} -> {role}")
            else:
                    ...
                    if alg == "RS256":
                        ...
                        if claims:
                            ...
                            if not auth_ctx.organization_id:
                                # Try X-Organization-ID first, then X-Tenant-Id (same entity, different names)
                                header_org_id = request.headers.get(ORG_ID_HEADER) or request.headers.get(TENANT_ID_HEADER)
                                if header_org_id:
                                    logger.info(f"[AuthMiddleware] Using org header: {header_org_id}")
