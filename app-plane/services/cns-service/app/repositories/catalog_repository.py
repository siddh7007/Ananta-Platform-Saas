...
class CatalogRepository:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        lifecycle_counts = dict(
            self.db.query(
                CatalogComponent.lifecycle_status,
                func.count(CatalogComponent.id)
            ).group_by(CatalogComponent.lifecycle_status).all()
        )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        """
        Get comprehensive catalog statistics for the /catalog/stats endpoint.

        Returns:
            Dictionary with total_components, by_lifecycle, by_quality_range,
            by_source, and average_quality.
        """
        total = self.count_total()
        avg_quality = self.db.query(
            func.avg(CatalogComponent.quality_score)
        ...
        # Clean up None keys for lifecycle
        if None in lifecycle_counts:
            lifecycle_counts["unknown"] = lifecycle_counts.pop(None)
        ...
        quality_data = self.db.query(
            CatalogComponent.quality_score
        ).filter(CatalogComponent.quality_score.isnot(None)).all()

        for (score,) in quality_data:
        ...
        # Source/enrichment source distribution
        source_counts = dict(
            self.db.query(
                CatalogComponent.enrichment_source,
                func.count(CatalogComponent.id)
            ).group_by(CatalogComponent.enrichment_source).all()
        )
        # Clean up None keys
        if None in source_counts:
            source_counts["unknown"] = source_counts.pop(None)

        return {
            "total_components": total or 0,
            "by_lifecycle": lifecycle_counts,
            "by_quality_range": quality_ranges,
