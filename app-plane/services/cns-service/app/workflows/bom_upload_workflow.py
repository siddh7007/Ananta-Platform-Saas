...
class BOMUploadProcessWorkflow:
    ...
    async def run(self, request: BOMUploadProcessRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            # Publish queue stage completion events for logging/audit
            try:
                await workflow.execute_activity(
                    publish_stage_completion_events,
                    {
                        'bom_upload_id': request.bom_upload_id,
                        'organization_id': request.organization_id,
                        'filename': bom_upload.get('filename', ''),
                        'total_rows': bom_upload.get('total_rows', 0),
                        'line_items_count': line_items_count,
                        'upload_source': upload_source
                    },
                    start_to_close_timeout=timedelta(seconds=10)
                )
                workflow.logger.info("✅ Published stage completion events")
            except Exception as e:
                workflow.logger.warning(f"Failed to publish stage completion events: {e}")
...
@activity.defn
async def publish_stage_completion_events(params: Dict[str, Any]) -> None:
    """
    Publish queue stage completion events for upload and parsing stages.

    This activity publishes RabbitMQ events for:
    - workflow.stage.raw_upload.completed
    - workflow.stage.parsing.completed

    These events are used for audit/logging and monitoring the BOM processing pipeline.
    """
    try:
        import sys
        import os
        ...
        bom_id = params.get('bom_upload_id')
        organization_id = params.get('organization_id')
        filename = params.get('filename', '')
        total_rows = params.get('total_rows', 0)
        line_items_count = params.get('line_items_count', 0)

        # Publish upload stage completed
        try:
            EventPublisher.workflow_upload_completed(
                bom_id=bom_id,
            ...
            logger.info(f"[Event] workflow.stage.raw_upload.completed published: bom_id={bom_id}")
        except Exception as e:
            logger.warning(f"Failed to publish upload stage event: {e}")

        # Publish parsing stage completed
        try:
            EventPublisher.workflow_parsing_completed(
                bom_id=bom_id,
                organization_id=organization_id,
                user_id=None,
                total_items=total_rows,
                valid_items=line_items_count,
                invalid_items=max(0, total_rows - line_items_count),
                duration_ms=None
            )
            logger.info(f"[Event] workflow.stage.parsing.completed published: bom_id={bom_id}")
        except Exception as e:
            logger.warning(f"Failed to publish parsing stage event: {e}")

    except Exception as e:
        logger.warning(f"Failed to publish stage completion events: {e}")
