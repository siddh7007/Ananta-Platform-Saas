...
class MinIOClient:
    ...
    def get_presigned_url(
        ...
        If MINIO_PUBLIC_ENDPOINT is configured, replaces the internal endpoint
        in the presigned URL with the public endpoint for browser accessibility.
        ...
        try:
            ...
            # e.g., "minio:9000" -> "localhost:27040"
            ...
            if public_endpoint and internal_endpoint and public_endpoint != internal_endpoint:
                # Handle both http:// prefixed and non-prefixed endpoints
                internal_host = internal_endpoint.replace("http://", "").replace("https://", "")
                public_host = public_endpoint.replace("http://", "").replace("https://", "")

                # Replace in URL
                if internal_host in url:
                    url = url.replace(internal_host, public_host)
                    logger.debug(f"Replaced internal endpoint '{internal_host}' with public '{public_host}' in presigned URL")
