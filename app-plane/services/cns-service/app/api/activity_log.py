...
def get_activity_log(
    ...
    organization_id: Optional[str] = Query(
        None, description="Filter by organization ID (super_admins only; others see their org)"
    ),
    ...
    Authorization:
    - Super admins can view all organizations or filter by specific org
    - Non-super-admins can only see their own organization's logs
    - Platform Super Admin org logs are visible to all authenticated staff
    """
    # Organization access control
    # Super admins can view any org; others are restricted to their own org
    ...
    if not auth.is_super_admin:
        if organization_id and organization_id != auth.organization_id and organization_id != PLATFORM_SUPER_ADMIN_ORG:
            logger.warning(f"[Activity Log] Non-super-admin tried to access other org: user={auth.user_id} requested={organization_id}")
            raise HTTPException(status_code=403, detail="Cannot view activity logs for other organizations")
        # If no org specified, default to user's org (but also include Platform Super Admin org)
        if not organization_id:
            effective_org_id = auth.organization_id
            logger.info(f"[Activity Log] Filtering to user's org: {effective_org_id}")
    ...
    # Use effective_org_id for filtering (enforces access control)
    # For non-super-admins, also include Platform Super Admin org in results
    include_platform_admin_org = not auth.is_super_admin and effective_org_id != PLATFORM_SUPER_ADMIN_ORG
    filters = {
        "organization_id": effective_org_id,
        "include_platform_admin_org": include_platform_admin_org,
        "platform_admin_org_id": PLATFORM_SUPER_ADMIN_ORG if include_platform_admin_org else None,
        "job_id": job_id,
        "bom_name": bom_name,
...
def _fetch_supabase_audit_logs(
    ...
    try:
        ...
        # Also include Platform Super Admin org if specified (for non-super-admins)
        if filters.get("organization_id"):
            if filters.get("include_platform_admin_org") and filters.get("platform_admin_org_id"):
                sql += " AND ((event_data->>'organization_id' = :org_id OR event_data->>'org_id' = :org_id) OR (event_data->>'organization_id' = :platform_org_id OR event_data->>'org_id' = :platform_org_id))"
                params["org_id"] = filters["organization_id"]
                params["platform_org_id"] = filters["platform_admin_org_id"]
            else:
...
def _fetch_enrichment_logs(
    ...
    for row in result:
        ...
        # Include Platform Super Admin org if specified (for non-super-admins)
        if filters.get("organization_id"):
            upload_org = upload_meta.get("organization_id")
            if upload_org != filters["organization_id"]:
                # Check if this is Platform Super Admin org (should be included)
                if filters.get("include_platform_admin_org") and filters.get("platform_admin_org_id"):
                    if upload_org != filters["platform_admin_org_id"]:
                        continue
                else:
                    continue
