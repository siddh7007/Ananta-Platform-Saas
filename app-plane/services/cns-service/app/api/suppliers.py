...
class SupplierSearchRequest(BaseModel):
    ...
    API expects simple strings for mpn and manufacturer, not complex validation objects.
    Validation is applied via Pydantic field validators.
    """
    mpn: str = Field(..., description="Manufacturer part number to search for", min_length=1, max_length=100)
    manufacturer: Optional[str] = Field(None, description="Optional manufacturer name", max_length=200)
    ...
    def validate_mpn(cls, v):
        """Validate and sanitize MPN using MPNValidator"""
        from app.core.input_validation import MPNValidator
        if not MPNValidator.validate(v):
            raise ValueError(f"Invalid MPN format: {v}")
        return MPNValidator.sanitize(v)

    @validator("manufacturer")
    def validate_manufacturer(cls, v):
        """Sanitize manufacturer name for XSS prevention"""
        if v is None:
            return v
...
def search_suppliers(
    ...
    """
    try:
        # Request model now uses plain strings with inline validators
        ...
        logger.info(f"[OK] Supplier search completed: {request.mpn} - {total_results} results from {len(search_results)} suppliers")

        return SupplierSearchResponse(
            query_mpn=request.mpn,
            query_manufacturer=request.manufacturer,
...
def get_product_details(
    ...
    try:
        ...
        logger.info(f"[OK] Product details found: {mpn_str} from {product_data.supplier_name}")
...
def get_best_match(
    ...
    try:
        ...
        logger.info(f"[OK] Best match found: {mpn} from {product_data.supplier_name} (confidence: {product_data.match_confidence}%)")
...
def check_supplier_health(
    ...
    try:
        ...
        logger.info(f"[OK] Supplier health check: {len(suppliers_available)} available, {len(suppliers_unavailable)} unavailable")
...
def exchange_digikey_oauth_code(
    ...
    try:
        ...
        logger.info(f"[OK] DigiKey OAuth tokens obtained successfully")
...
def refresh_digikey_oauth_token(
    ...
    try:
        ...
        logger.info("[OK] DigiKey OAuth token refreshed via API")
