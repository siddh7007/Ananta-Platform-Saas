...
# Scope decorators for BOM endpoint protection
...
async def confirm_bom_mapping(
    ...
    logger.info(f"User confirmed mappings for job {job_id}: {data.column_mappings}")
    ...
    try:
        ...
        current_metadata['confirmed_mappings'] = data.column_mappings
    ...
    # Trigger Temporal workflow for BOM processing (comprehensive pipeline)
    workflow_id = None
    try:
        # Import new Temporal client helper
        from app.workflows.temporal_client import start_bom_processing
        ...
        # This workflow handles: Upload → Parse → Enrich → Risk Analysis → Complete
        ...
        logger.info(f"✅ BOM Processing Workflow started: {workflow_id} for job: {job_id}")
        logger.info(f"   Stages: Upload → Parsing → Enrichment → Risk Analysis → Complete")
        ...
        try:
            ...
            # Note: user_email and user_id_from_meta already extracted above
        ...
        # [LEGACY_FALLBACK] Thread-based enrichment - Only when Temporal is unavailable
        logger.warning(
            "[LEGACY_FALLBACK] Falling back to threading for background processing",
            extra={
                'job_id': job_id,
                'organization_id': organization_id,
                'flow': 'LEGACY_FALLBACK_THREADING'
            }
        )
        ...
        def process_bom_job_background(job_id, items, organization_id):
            ...
            try:
                ...
                try:
                    ...
                    # HARDENING: Update boms.enrichment_status to keep Supabase in sync
                    # job_id == bom_id in this flow (see line 643)
                    try:
                        bom_enrichment_status = 'enriched' if failed_count == 0 else 'requires_approval'
                        thread_db.execute(text("""
                            UPDATE boms
                            ...
                            WHERE id = :bom_id
                        """), {
                            "enrichment_status": bom_enrichment_status,
                            "bom_status": "ready" if failed_count == 0 else "needs_review",
                            "bom_id": job_id
                        })
                        thread_db.commit()
                        debug_log(f"   ✅ BOM enrichment_status set to '{bom_enrichment_status}'")
                        logger.info(f"[LEGACY_FALLBACK] Updated boms.enrichment_status='{bom_enrichment_status}' for bom_id={job_id}")
                    except Exception as bom_err:
                        logger.error(f"[LEGACY_FALLBACK] Failed to update boms.enrichment_status: {bom_err}")
                        debug_log(f"   ⚠️ Failed to update BOM enrichment_status: {bom_err}")
                        thread_db.rollback()
                ...
                try:
                    ...
                    # HARDENING: Update boms.enrichment_status to 'failed'
                    try:
                        thread_db.execute(text("""
                            UPDATE boms
                            SET enrichment_status = 'failed',
                        ...
                        """), {"bom_id": job_id})
                        thread_db.commit()
                        debug_log(f"   ✅ BOM enrichment_status set to 'failed'")
                        logger.info(f"[LEGACY_FALLBACK] Updated boms.enrichment_status='failed' for bom_id={job_id}")
                    except Exception as bom_err:
                        logger.error(f"[LEGACY_FALLBACK] Failed to update boms.enrichment_status: {bom_err}")
                        thread_db.rollback()
...
async def cancel_job(
    ...
    try:
        ...
        # Update job status in database (db parameter from decorator)
        update_query = text("""
            UPDATE bom_jobs
...
async def retry_failed_items(
    ...
    try:
        # Get original job from db parameter (from decorator)
        job_query = text("""
