...
async def list_enrichment_latest(
    ...
    try:
        ...
        # Calculate progress from bom_line_items.enrichment_status instead of
        # enrichment_events.state (which is often empty)
        sql = f"""
            WITH latest AS (
            ...
            SELECT
                ...
                b.component_count,
                -- Calculate status from line items if available, otherwise fall back
                CASE
                    WHEN COALESCE(lis.total_items, 0) = 0 THEN COALESCE(latest.state->>'status', b.status, 'pending')
                    WHEN lis.enriched_items + lis.failed_items >= lis.total_items THEN 'completed'
                    WHEN lis.enriched_items > 0 OR lis.failed_items > 0 THEN 'enriching'
                    ELSE COALESCE(latest.state->>'status', b.status, 'pending')
                END AS status,
                -- Calculate percent_complete from line items
                CASE
                    WHEN COALESCE(lis.total_items, 0) = 0 THEN COALESCE((latest.state->>'percent_complete')::numeric, 0)
                    ELSE ROUND(((lis.enriched_items + lis.failed_items)::numeric / lis.total_items) * 100, 1)
                END AS percent_complete,
                COALESCE(lis.total_items, (latest.state->>'total_items')::integer, b.component_count, 0) AS total_items,
                COALESCE(lis.enriched_items, (latest.state->>'enriched_items')::integer, 0) AS enriched_items,
                COALESCE(lis.failed_items, (latest.state->>'failed_items')::integer, 0) AS failed_items,
            ...
            LEFT JOIN line_item_stats lis ON lis.bom_id = b.id
            LEFT JOIN LATERAL (
                SELECT
                    bu.upload_source,
                    bu.filename,
...
# NOTE: /line-items/count MUST be defined BEFORE /line-items/{item_id}
# otherwise FastAPI will match "count" as an item_id
...
async def get_line_item(
    ...
    Requires ADMIN role. Super admins can access any item,
    regular admins can only access items in their organization.
    """
    try:
        logger.info(f"[Admin] get_line_item: user={auth.user_id} role={auth.role} item_id={item_id}")
        db = next(get_dual_database().get_session("supabase"))

        filters = ["bli.id = :item_id"]
        ...
        if not row:
            raise HTTPException(status_code=404, detail=f"Line item not found: {item_id}")

        return {
            "id": str(row[0]),
            "organization_id": str(row[1]) if row[1] else None,
            "project_id": str(row[2]) if row[2] else None,
            "bom_id": str(row[3]) if row[3] else None,
            "line_number": row[4],
            "manufacturer_part_number": row[5],
            "manufacturer": row[6],
            "description": row[7],
            "enrichment_status": row[8],
            "component_id": str(row[9]) if row[9] else None,
            "lifecycle_status": row[10],
            "datasheet_url": row[11],
            "specifications": row[12],
            "compliance_status": row[13],
            "pricing": row[14],
            "enriched_at": row[15].isoformat() if row[15] else None,
            "quantity": row[16],
            "reference_designator": row[17],
        ...
        logger.error(f"Failed to get line item: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Failed to get line item")
            ...
            output.append(
                {
                    ...
                    "timestamp": mapping["created_at"].isoformat() if mapping["created_at"] else None,
