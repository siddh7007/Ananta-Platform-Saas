...
# CNS Projects Alignment - Phase 3: Workspace Scope Validation
from app.core.scope_decorators import require_workspace
from app.dependencies.scope_deps import get_supabase_session, get_supabase_session_cm
...
async def get_workspace_summary(
    organization_id: str = Query(..., description="Organization ID to get summary for"),
    user: User = Depends(get_current_user)
):
    """
    Get workspace summary for dashboard.

    Returns counts of workspaces, projects, and BOMs for the organization,
    along with recently updated workspaces and projects.
    """
    logger.info(f"[Workspaces] Getting summary for org={organization_id}, user={user.email}")

    with get_supabase_session_cm() as session:
        # Get total workspace count
        total_workspaces = session.execute(
            text("""
        ...
        # Get recent workspaces (last 5 updated)
        recent_ws_rows = session.execute(
            text("""
                SELECT
        ...
        recent_workspaces = [
            ...
            for row in recent_ws_rows
        ]

        # Get recent projects (last 5 updated)
        recent_proj_rows = session.execute(
            text("""
                SELECT
                    p.id, p.name, p.workspace_id, p.status, p.created_at, p.updated_at,
        ...
        recent_projects = [
            RecentProject(
                ...
                bomCount=row.bom_count or 0,
                status=row.status or "active",
                createdAt=row.created_at.isoformat() if row.created_at else None,
                updatedAt=row.updated_at.isoformat() if row.updated_at else None
            )
            for row in recent_proj_rows
        ]

        return WorkspaceSummaryResponse(
            totalWorkspaces=total_workspaces,
            totalProjects=total_projects,
            totalBoms=total_boms,
...
async def list_workspaces(
    ...
    Platform admins (is_platform_admin=True) and org admins/owners can see ALL workspaces
    in the organization. Regular users only see workspaces they are members of.
    """
    with get_supabase_session_cm() as session:
        # Check if user is platform admin first
        is_platform_admin = user.is_platform_admin

        # Verify user is org member and get their role
        # Note: user.id is the Keycloak user ID (sub claim), not the database user ID
        # We need to join with users table to find the database user and their membership
        org_member = session.execute(
            text("""
                SELECT om.role, u.id as db_user_id, u.is_platform_admin, u.role as user_role
            ...
            """),
            {"org_id": organization_id, "keycloak_user_id": user.id}
        ).fetchone()

        # If not org member, check if platform admin can still access
        if not org_member:
            if is_platform_admin:
                # Platform admins can access any org - find their user ID
                user_row = session.execute(
                    text("""
                        SELECT id, role, is_platform_admin FROM users
                        WHERE keycloak_user_id = :keycloak_user_id
                           OR auth0_user_id = :keycloak_user_id
                           OR CAST(id AS TEXT) = :keycloak_user_id
                    """),
                    {"keycloak_user_id": user.id}
                ).fetchone()
                if user_row:
                    db_user_id = str(user_row.id)
                    org_role = "super_admin"  # Platform admins get super_admin access
                    user_role = user_row.role or "super_admin"
                    is_platform_admin = user_row.is_platform_admin or is_platform_admin
                else:
                    raise HTTPException(
                        status_code=status.HTTP_403_FORBIDDEN,
            ...
            db_user_id = str(org_member.db_user_id)
            org_role = org_member.role
            user_role = org_member.user_role
            is_platform_admin = is_platform_admin or (org_member.is_platform_admin or False)

        # Determine if user can see ALL workspaces (admins/owners/platform admins)
        # Role hierarchy: super_admin(5) > owner(4) > admin(3) > engineer(2) > analyst(1)
        ADMIN_ROLES = ('super_admin', 'owner', 'admin', 'org_admin')
        can_see_all_workspaces = is_platform_admin or org_role in ADMIN_ROLES or user_role in ADMIN_ROLES

        if can_see_all_workspaces:
            # Admins see ALL workspaces in the organization (with their role if member, or 'admin' if not)
            ...
            result = session.execute(
                text("""
                    SELECT
                        ...
                        w.settings, w.created_at, w.updated_at, w.is_default,
                        COALESCE(wm.role, 'admin') as role
                    FROM workspaces w
                ...
                """),
                {"org_id": organization_id, "user_id": db_user_id, "limit": limit, "offset": offset}
            )
        else:
            ...
            total = session.execute(
                text("""
                    ...
                    AND wm.user_id = CAST(:user_id AS UUID)
                """),
                {"org_id": organization_id, "user_id": db_user_id}
            ).scalar() or 0
...
async def create_workspace(
    ...
    with get_supabase_session_cm() as session:
        ...
        org_member = session.execute(
            text("""
                SELECT om.role, u.id as db_user_id FROM organization_memberships om
                JOIN users u ON u.id = om.user_id
                WHERE om.organization_id = CAST(:org_id AS UUID)
                AND (u.keycloak_user_id = :keycloak_user_id
                     OR u.auth0_user_id = :keycloak_user_id
                     OR CAST(u.id AS TEXT) = :keycloak_user_id)
            """),
            {"org_id": data.organization_id, "keycloak_user_id": user.id}
        ).fetchone()
        ...
        # Use the database user ID for subsequent queries
        db_user_id = str(org_member.db_user_id)

        # Use role hierarchy: super_admin(5) > owner(4) > admin(3) > engineer(2) > analyst(1)
        # Engineers and above can create workspaces
        ALLOWED_ROLES_FOR_WORKSPACE_CREATE = ('super_admin', 'owner', 'admin', 'org_admin', 'billing_admin', 'engineer')
        if org_member.role not in ALLOWED_ROLES_FOR_WORKSPACE_CREATE:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Engineers or higher role required to create workspaces"
            )
        ...
        session.execute(
            ...
            """),
            {"ws_id": workspace_id, "user_id": db_user_id}
        )
...
async def get_workspace(
    ...
    **Phase 3: CNS Projects Alignment**
    ...
    Security:
        - Server derives organization_id from validated workspace FK chain
    ...
    logger.info(
        f"[Workspaces] Getting workspace {workspace_id} "
...
async def update_workspace(
    ...
    """
    Update workspace details with automatic scope validation.
    ...
    **Phase 3: CNS Projects Alignment**
    ...
    Authorization:
        - Automatic validation: workspace → organization
        - Users can only update workspaces in their organization
    ...
    logger.info(
        f"[Workspaces] Updating workspace {workspace_id} "
        f"(org={organization_id}, user={user.id})"
    )

    # Get workspace with user role (already validated by decorator)
    workspace = db.execute(
        text("""
            SELECT w.*, wm.role as user_role
            FROM workspaces w
    ...
    # Require admin role (workspace admin or org admin/owner/super_admin)
    # Role hierarchy: super_admin(5) > owner(4) > admin(3) > engineer(2) > analyst(1)
    ADMIN_ROLES = ('super_admin', 'owner', 'admin', 'org_admin')
    workspace_role = workspace.user_role

    # Check if user has admin-level access (workspace admin or org-level admin+)
    if workspace_role not in ADMIN_ROLES:
        # Also check organization membership for elevated roles
        # Note: user.id is the Keycloak user ID (auth0_user_id), not the database ID
        # So we need to join with users table to find the correct membership
        org_role_result = db.execute(
            text("""
                SELECT om.role FROM organization_memberships om
                JOIN users u ON u.id = om.user_id
                WHERE om.organization_id = CAST(:org_id AS UUID)
                AND (u.auth0_user_id = :keycloak_user_id OR u.id = CAST(:keycloak_user_id AS UUID))
            """),
            {"org_id": str(workspace.organization_id), "keycloak_user_id": user.id}
        ).fetchone()

        org_role = org_role_result.role if org_role_result else None
        if org_role not in ADMIN_ROLES:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Only workspace or organization admins can perform this action"
            )
...
async def delete_workspace(
    ...
    """
    Delete a workspace (soft delete) with automatic scope validation.
    ...
    **Phase 3: CNS Projects Alignment**
    ...
    Authorization:
        - Automatic validation: workspace → organization
        - Users can only delete workspaces in their organization
        - Requires workspace admin role
        - Staff users can bypass scope validation
    ...
    logger.info(
        f"[Workspaces] Deleting workspace {workspace_id} "
        f"(org={organization_id}, user={user.id})"
    )
    ...
    # Note: workspace_invitations table may not exist in all deployments
    # The schema uses organization_invitations instead
    ...
    # Hard delete workspace (no deleted_at column in schema)
    db.execute(
        text("""
...
async def list_members(
    ...
    """
    workspace = get_workspace_or_404(db, workspace_id, user.id)
