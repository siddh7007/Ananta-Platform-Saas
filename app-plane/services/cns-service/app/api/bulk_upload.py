...
async def upload_bulk_file(
    ...
    # Exception: Any authenticated staff can upload to Platform Super Admin org (shared test/demo org)
    is_platform_admin_org = organization_id == PLATFORM_SUPER_ADMIN_ORG
    if not auth.is_super_admin and auth.organization_id != organization_id and not is_platform_admin_org:
    ...
    if is_platform_admin_org and auth.organization_id != organization_id:
        logger.info(f"[Admin] Upload to Platform Super Admin org allowed via bypass: user={auth.user_id}")

    try:
        # [FLOW_4] Bulk Staff Upload - CNS Dashboard staff
        logger.info(
            f"[FLOW_4][Bulk Staff Upload] Starting upload: {file.filename} by user={auth.user_id}",
            extra={
                'upload_id': upload_id,
                'organization_id': organization_id,
                'flow': 'FLOW_4_BULK_STAFF',
        ...
        try:
            ...
            try:
                ...
                """)
                    ...
                    # Add timestamp to BOM name to ensure uniqueness (allows re-uploading same file)
                    timestamp = datetime.utcnow().strftime("%Y%m%d_%H%M%S")
                    unique_bom_name = f"Bulk Upload - {file.filename} ({timestamp})"

                    supabase_db.execute(bom_insert_query, {
                        "bom_id": bom_id,
                        "bom_name": unique_bom_name,
                        "organization_id": organization_id,
