 ...
 *
 * IMPORTANT: This component does NOT use react-admin hooks (useLogin, useNotify)
 * because those hooks require Router context which may not be available when
 * Auth0 redirects back after authentication. Instead, it relies on:
 * 1. The authProvider's checkAuth method to detect Auth0 authentication
 * 2. React Admin's automatic redirect after successful checkAuth
 *
 * Supported providers:
 * - Google
 * - Microsoft
...
export interface Auth0LoginProps {
  ...
  /**
   * Force platform admin login flow even if route isn't /admin-login
   */
  forcePlatformOrgLogin?: boolean;
}
...
export const Auth0Login: React.FC<Auth0LoginProps> = ({
  ...
  forcePlatformOrgLogin = false,
}) => {
  const { loginWithRedirect, isAuthenticated, isLoading, user, error, logout } = useAuth0();

  // Determine if platform admin login should be enforced
  const isAdminLogin =
      ...
      >
        <Card sx={{ maxWidth: 450, width: '100%', boxShadow: 3 }}>
          <CardContent sx={{ p: 4 }}>
            ...
            {/* Login Button */}
            {/* Button is disabled only during first 3s loading, then enabled via loadingTimedOut */}
            <Button
              variant="contained"
              fullWidth
              size="large"
