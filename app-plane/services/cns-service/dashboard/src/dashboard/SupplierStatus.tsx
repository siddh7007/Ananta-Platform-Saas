...
import { Card, CardContent, Typography, Box, LinearProgress, Chip, Tooltip, useTheme } from '@mui/material';
import CheckCircleIcon from '@mui/icons-material/CheckCircle';
import WarningIcon from '@mui/icons-material/Warning';
import ErrorIcon from '@mui/icons-material/Error';
import { getSupplierColor, enrichmentStatusColors, alertSeverityColors } from '../theme';
...
const statusConfig: Record<string, { icon: React.ReactNode; label: string }> = {
  healthy: { icon: <CheckCircleIcon fontSize="small" sx={{ color: enrichmentStatusColors.completed }} />, label: 'Healthy' },
  'rate-limited': { icon: <WarningIcon fontSize="small" sx={{ color: alertSeverityColors.warning }} />, label: 'Rate Limited' },
  error: { icon: <ErrorIcon fontSize="small" sx={{ color: alertSeverityColors.error }} />, label: 'Error' },
};
                ...
                <Box display="flex" justifyContent="space-between" alignItems="center" mb={0.5}>
                  ...
                  <Tooltip title={`${supplier.requestsUsed}/${supplier.requestsLimit} requests used`}>
                    <Chip
                      ...
                      size="small"
                      sx={{
                        backgroundColor: isLow ? `${alertSeverityColors.warning}20` : theme.palette.grey[100],
                        color: isLow ? alertSeverityColors.warning : theme.palette.grey[700],
                        fontWeight: 600,
                        fontSize: '0.7rem',
                      }}
                    />
                ...
                <LinearProgress
                  ...
                  sx={{
                    ...
                    '& .MuiLinearProgress-bar': {
                      backgroundColor: isLow ? alertSeverityColors.error : color,
                      borderRadius: 3,
                    },
                  }}
                />
