...
import { waitForAuth0Ready, resetAuth0State, getAuth0State } from './auth0State';
...
export interface Auth0AuthProviderConfig {
   ...
   * @default 'a0000000-0000-0000-0000-000000000000'
   */
...
export function createAuth0AuthProvider(config: Auth0AuthProviderConfig): AuthProvider {
  const {
    ...
    defaultTenantId: _defaultTenantId = 'a0000000-0000-0000-0000-000000000000',
  ...
  /**
   * Derive system role from Auth0 organization roles
   * Maps platform:admin, platform:staff, etc. to system roles
   */
  const deriveRoleFromAuth0Roles = (auth0Roles: string[]): string => {
    if (!auth0Roles || !Array.isArray(auth0Roles) || auth0Roles.length === 0) {
      return 'member';
    }

    // Map Auth0 org roles to system roles (same logic as backend)
    ...
    if (auth0Roles.some(r => ['platform:engineer', 'engineer'].includes(r))) {
      return 'engineer';
    }
  ...
  return {
    ...
    logout: async () => {
      ...
      if (!hasActiveSession) {
        ...
        // DON'T reset Auth0 state here - it might be in the process of syncing
        // from Auth0StateSync. Resetting would undo the sync and cause a loop.
        // The Auth0 SDK state is the source of truth.
    ...
    checkAuth: async () => {
      ...
      try {
        ...
        if (auth0State.isAuthenticated && auth0State.user) {
          // GLOBAL ROLE CHECK: Verify user has an allowed role on ALL routes (if configured)
          // This is a security measure for platform admin dashboards (CNS, Backstage)
          if (allowedRoles && allowedRoles.length > 0) {
            ...
            // FALLBACK: If user is a member of the platform organization, allow access
            // This handles the case where Auth0 RBAC roles haven't been assigned yet
            const isPlatformOrgMember = auth0OrgId === platformOrgId;

            if (!hasAllowedRole && !isPlatformOrgMember) {
              // Check if we should force re-authentication with organization parameter
              // This happens when user has a session without org_id (logged in before org was configured)
              const notAlreadyReauthenticating = !sessionStorage.getItem('platform_org_reauth_attempted');

              if (!auth0OrgId && notAlreadyReauthenticating) {
                log('No org_id in token - forcing re-authentication with platform organization', {
                  email: auth0State.user.email
                });
                sessionStorage.setItem('platform_org_reauth_attempted', 'true');

                // Clear Auth0 SDK cache to force fresh login
                try {
                  const keysToRemove: string[] = [];
                  for (let i = 0; i < localStorage.length; i++) {
                    const key = localStorage.key(i);
                ...
                return Promise.reject(new Error('PLATFORM_ORG_REAUTH'));
              }

              error('ACCESS DENIED: User does not have required platform role or org membership', {
              ...
              localStorage.removeItem('organization_id');
              localStorage.removeItem('auth0_access_token');
              // Clear re-auth flag so user can try again
              sessionStorage.removeItem('platform_org_reauth_attempted');
              return Promise.reject(new Error('ADMIN_LOGIN_REQUIRES_ROLE'));
            }

            // Clear re-auth flag on successful auth
            sessionStorage.removeItem('platform_org_reauth_attempted');

            if (isPlatformOrgMember && !hasAllowedRole) {
              log('Platform org member without RBAC roles - granting access via org membership', {
                auth0OrgId,
                email: auth0State.user.email
              });
            } else {
              log('Global role check passed', { userRoles: auth0Roles, allowedRoles });
            }
          }
          ...
          // ALWAYS update user_role based on Auth0 org roles (frontend display)
          const derivedRole = deriveRoleFromAuth0Roles(auth0Roles);
          const currentRole = localStorage.getItem('user_role');
          if (derivedRole !== 'member' && currentRole !== derivedRole) {
            localStorage.setItem('user_role', derivedRole);
            log('Updated user_role from Auth0 org roles:', { currentRole, derivedRole, auth0Roles });
          }
          ...
          if (userId && userEmail) {
            ...
            if (useDirectAuth0JWT) {
              ...
              // Derive role: first from middleware, then from Auth0 custom claim, then from Auth0 org roles
              const finalRole = userData?.role || auth0Role || deriveRoleFromAuth0Roles(auth0Roles);
              const finalUserId = userData?.id || sub;
              log('Derived finalRole:', { middlewareRole: userData?.role, auth0Role, auth0Roles, finalRole });
              ...
              localStorage.setItem('is_admin', userData?.is_platform_admin ? 'true' : (auth0OrgId === platformOrgId ? 'true' : 'false'));
            ...
            try {
              ...
              if (sessionResponse.ok) {
                ...
                if (sessionData.user) {
                  ...
                  localStorage.setItem('user_role', sessionData.user.role || 'member');
    ...
    getPermissions: async () => {
      ...
      // First check localStorage (fast path for existing sessions)
      ...
      // localStorage empty - check Auth0 state synchronously first
      const currentAuth0State = getAuth0State();

      // If Auth0 is done loading and NOT authenticated, reject immediately (no waiting)
      if (!currentAuth0State.isLoading && !currentAuth0State.isAuthenticated) {
        log('Auth0 loaded but not authenticated - rejecting immediately');
        return Promise.reject(new Error('No active session'));
      }

      // Auth0 is still loading OR is authenticated but localStorage not synced yet
      // Wait for Auth0 state to stabilize
      log('Waiting for Auth0 state sync...');
      try {
        const auth0State = await waitForAuth0Ready(5000, 1, useDirectAuth0JWT);
        if (auth0State.isAuthenticated && auth0State.user) {
          // Auth0 is authenticated - checkAuth should populate localStorage soon
          // Return a pending state rather than rejecting
          log('Auth0 authenticated, checkAuth will populate localStorage');
          return Promise.resolve('pending');
        }
      } catch {
        // Auth0 not ready, fall through to rejection
      }

      // Re-check localStorage in case checkAuth ran
      userId = localStorage.getItem('user_id');
      userEmail = localStorage.getItem('user_email');

      if (userId && userEmail) {
        const role = localStorage.getItem('user_role') || 'member';
        log('User permissions retrieved after sync', { role });
        return Promise.resolve(role);
      }
    ...
    getIdentity: async () => {
      ...
      if (userId && userEmail) {
        // Fast path: localStorage has user data
        const identity = {
          id: userId,
      ...
      try {
        const auth0State = await waitForAuth0Ready(5000, 1, useDirectAuth0JWT);
        if (auth0State.isAuthenticated && auth0State.user) {
          // Auth0 is authenticated - use Auth0 user data directly as fallback
          const user = auth0State.user;
          log('Using Auth0 user data directly', { email: user.email });

          // Return identity from Auth0 state while checkAuth populates localStorage
          return Promise.resolve({
            id: user.sub || 'pending',
      ...
      if (userId && userEmail) {
        ...
        log('User identity retrieved after sync', { userId: identity.id });
        return Promise.resolve(identity);
      }
