...
/**
 * Enrichment Status Badge Wrapper for React Admin
 */
const EnrichmentStatusField: React.FC<{ source?: string }> = ({ source = 'enrichment_status' }) => {
  const record = useRecordContext();
  if (!record) return null;

  const status = String(record[source] || 'pending').toLowerCase();

  // Map React Admin enrichment statuses to shared component statuses
  ...
  return <EnrichmentStatusBadge status={statusMap[status] || status} variant="chip" size="small" />;
};
    ...
    <Stack direction="row" spacing={0.5} justifyContent="center">
      ...
      {record.matched_component_id && (
        <Tooltip title="View Linked Component">
          <IconButton
            aria-label="View linked component"
            size="small"
            color="success"
            onClick={(e) => {
              e.stopPropagation();
              window.location.hash = `#/component/${record.matched_component_id}`;
            }}
          >
...
export const BOMLineItemList: React.FC = () => (
  ...
  >
    <DatagridConfigurable
      aria-label="BOM line items table"
      rowClick="edit"
    ...
    >
      ...
      <EnrichmentStatusField source="enrichment_status" />
      <LineItemActions />
    </DatagridConfigurable>
  </List>
);
