...
export const BOMJobDetail: React.FC = () => {
  ...
  const redirectToEnrichmentMonitor = (bomId: string) => {
  ...
  const loadStatus = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const message = errorData?.detail || 'Failed to load job status';
        if (typeof message === 'string' && message.toLowerCase().includes('bom_id')) {
          setError('This link references a BOM ID. Redirecting to Enrichment Monitor...');
          redirectToEnrichmentMonitor(jobId);
          return;
        }
