...
export interface UseEnrichmentPollingReturn {
  ...
  /** Whether polling is connected (no recent failures) */
  isConnected: boolean;
}
...
export function useEnrichmentPolling(
  ...
  const fetchStatus = useCallback(async () => {
    ...
    try {
      ...
      // Map API status to UI status (API uses "processing", UI expects "enriching")
      const rawStatus = data.enrichment_status || data.status || 'pending';
      const mappedStatus = rawStatus === 'processing' ? 'enriching' : rawStatus;

      const newState: EnrichmentState = {
        status: mappedStatus as EnrichmentState['status'],
      ...
      if (newCount >= 3) {
        ...
        // Show user notification only once (don't spam on every failure)
        if (!notificationShownRef.current) {
          console.error('[CNS Polling] Connection lost after 3 failures. Unable to fetch enrichment status.');
          notificationShownRef.current = true;

          // User notification will be shown by consumers via isConnected state
          // This keeps the hook UI-agnostic while still tracking the state
        }
      }

      console.error(`[CNS Polling] Error fetching status (attempt ${newCount}):`, err);
