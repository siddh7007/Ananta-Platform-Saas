...
import { WatchedComponentsPage } from './pages/WatchedComponents';

// P0 Features - New Components
import { PortfolioDashboardPage } from './pages/PortfolioDashboardPage';
import { SmartColumnMapperPage } from './pages/SmartColumnMapperPage';

// P4 Features - Notification Inbox
    ...
    <Menu>
      ...
      {isAdmin && (
        <>
          ...
          <Menu.Item
            to="/portfolio"
            primaryText="Portfolio Dashboard"
...
const AUTH_PROVIDER = import.meta.env.VITE_AUTH_PROVIDER?.toLowerCase() || 'supabase';

// Get the login page based on auth provider
const getLoginPage = () => {
  if (AUTH_PROVIDER === 'auth0') return Auth0Login;
  if (AUTH_PROVIDER === 'keycloak') return KeycloakLogin;
  return DevModeLogin;
};

// Get the auth provider based on configuration
const getAuthProvider = () => {
  if (AUTH_PROVIDER === 'keycloak') return keycloakAuthProvider;
  return authProvider; // Default auth provider handles auth0/supabase/dev modes
};
 ...
 * - supabase: Supabase auth (dev mode)
 */
function ThemedAdmin() {
  ...
  return (
    <OrganizationProvider>
      <WorkspaceProvider>
        <Admin
          key={`admin-theme-${mode}-${AUTH_PROVIDER}`}  // Force re-render when theme mode or auth changes
      ...
      <CustomRoutes>
        ...
        <Route path="/alerts/watched" element={<WatchedComponentsPage />} />
        <Route path="/notifications" element={<AlertCenter />} /> {/* Alias for /alerts */}
        <Route path="/inbox" element={<NotificationInbox />} /> {/* Full notification inbox */}
        <Route path="/theme" element={<AccountSettings />} /> {/* Theme settings in account */}
        {/* P0 Features - New Routes */}
