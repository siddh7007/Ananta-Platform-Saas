...
import { getCnsBaseUrl, getAuthHeaders, cnsApi } from '../services/cnsApi';
...
// Feature flag: when true, use the new project-scoped upload endpoint
// /api/boms/projects/{project_id}/boms/upload for better security
// This endpoint automatically derives organization_id from project FK chain,
// preventing cross-tenant data leakage. Enabled by default for production.
// Set VITE_USE_SCOPED_UPLOAD=false to disable and use legacy path.
const USE_SCOPED_UPLOAD = ['true', '1', 'yes'].includes(
  ((import.meta as any).env?.VITE_USE_SCOPED_UPLOAD || 'true').toLowerCase()
);
...
export const BOMUploadWorkflow: React.FC = () => {
            ...
            const queueItem: QueueItemData = {
              ...
              columnMappings: uploadRecord.column_mappings
                ? Object.entries(uploadRecord.column_mappings).map(([source, target]) => ({
                    ...
                    confidence: 100, // Restored from DB, assume confirmed
                  }))
                : [],
            };
            setQueue([queueItem]);
          ...
          const queueItem: QueueItemData = {
            ...
            columnMappings: uploadRecord.detected_columns
              ? Object.entries(uploadRecord.detected_columns).map(([source, target]) => ({
                  source,
                  target: target as any,
                  confidence: 80, // Auto-detected, medium confidence
                }))
              : [],
  ...
  const confirmMappings = async (queueIndex: number) => {
    ...
    try {
      ...
      // Path 1: Use new project-scoped upload endpoint (recommended)
      if (USE_SCOPED_UPLOAD && currentProjectId) {
        try {
          console.log('[BOMUpload] Using new scoped upload endpoint');

          const uploadResponse = await cnsApi.uploadBOMToProject(
            currentProjectId,
            item.file,
            {
              bomName: item.file.name,
              priority: 'normal',
              source: 'customer',
              uploadedBy: userEmail || dbUserId || undefined,
              startEnrichment: true,
              columnMappings: columnMappings,
            }
          );

          console.log('[BOMUpload] Scoped upload response:', uploadResponse);

          // Update bom_uploads with final status and BOM ID
          await supabase
            .from('bom_uploads')
            .update({
              status: 'completed',
              bom_id: uploadResponse.bom_id,
            })
            .eq('id', item.uploadId);
          ...
          setQueue((prev) =>
            prev.map((it, idx) =>
              idx === queueIndex
                ? { ...it, status: 'completed', bomId: uploadResponse.bom_id, allRows: undefined }
                : it
            )
          );

          // Track success
          analytics.trackBomUploadComplete(uploadResponse.bom_id, uploadResponse.component_count);
          ...
          await publishCustomEvent('customer.bom.uploaded', 'bom_uploaded', {
            ...
            bom_id: uploadResponse.bom_id,
            project_id: currentProjectId,
            organization_id: uploadResponse.organization_id,
            user_id: currentUserId,
            filename: item.file.name,
            status: 'completed',
            total_rows: uploadResponse.component_count,
            line_items_saved: uploadResponse.component_count,
            enrichment_started: uploadResponse.enrichment_started,
          }, 7);

          notify(
            `BOM uploaded successfully! ${uploadResponse.component_count} components. ${
              uploadResponse.enrichment_started ? 'Enrichment started.' : ''
            }`,
            { type: 'success' }
          );
          return;
        } catch (error: unknown) {
          const message = error instanceof Error ? error.message : 'Scoped upload failed';
          console.error('[BOMUpload] Scoped upload error:', message);

          // Fall back to legacy path on error
          notify(`New upload method failed: ${message}. Trying legacy method...`, { type: 'warning' });
        }
      }

      // Path 2: Use CNS snapshot path if enabled
