...
export interface Auth0AuthProviderConfig {
  ...
  /**
   * Default tenant ID for new users (Platform Super Admin organization)
...
export function createAuth0AuthProvider(config: Auth0AuthProviderConfig): AuthProvider {
  const {
    ...
    defaultTenantId = 'a0000000-0000-0000-0000-000000000000',
  ...
  return {
    login: async ({ auth0User, auth0AccessToken }: any) => {
      ...
      if (useDirectAuth0JWT) {
        ...
        if (!auth0OrganizationId) {
          ...
          error(`Ensure Auth0 Action is deployed and adds ${namespace}/organization_id`);
      ...
      // ========================================
        ...
        try {
          ...
          // Persist subscription info for feature gating/UX
          try {
            if (sessionData.subscription) {
              const sub = sessionData.subscription;
              if (sub.plan) localStorage.setItem('subscription_plan', sub.plan);
              if (sub.status) localStorage.setItem('subscription_status', sub.status);
              if (sub.trial_end) localStorage.setItem('subscription_trial_end', sub.trial_end);
              if (sub.current_period_end) localStorage.setItem('subscription_period_end', sub.current_period_end);
            }
          } catch {}
          ...
          // Store user data in localStorage
          // ⚠️ SECURITY WARNING (SEC-001): localStorage is vulnerable to XSS attacks
          // Session tokens are stored in Supabase SDK (also uses localStorage)
          // For production hardening, consider:
          //   1. Implementing httpOnly cookies for token storage
          //   2. Using a secure session backend
          //   3. Enabling strict CSP headers
          //   4. Implementing XSS protection middleware
          localStorage.setItem('user_id', userData.id);
          localStorage.setItem('user_email', userData.email);
          localStorage.setItem('organization_id', userData.organization_id);
          localStorage.setItem('user_role', userData.role);
    ...
    logout: async () => {
      ...
      try {
        ...
        localStorage.removeItem('subscription_trial_end');
        localStorage.removeItem('subscription_period_end');
      } catch {}

      // Clear Auth0 SDK localStorage keys (race condition fix)
      // Auth0 SDK stores tokens in keys like: @@auth0spajs@@::{clientId}::{audience}::{scope}
      try {
    ...
    checkAuth: async () => {
      ...
      try {
        ...
        // Production SPAs treat "Auth0 still loading" as transient state, not an error
        // For Option A (Direct Auth0 JWT), also wait for getAccessTokenSilently to be set by Auth0StateSync
        ...
        if (auth0State.isAuthenticated && auth0State.user) {
          ...
          // This ensures roles are updated when user re-authenticates via /admin-login
          const auth0Roles = auth0State.user[`${namespace}/roles`] || [];
          localStorage.setItem('auth0_roles', JSON.stringify(auth0Roles));
          log('Updated auth0_roles from token', { auth0Roles });

          if (userId && userEmail) {
            // ALWAYS sync auth_mode with current configuration to prevent stale values
            // This fixes the issue where previous auth0_direct mode persists incorrectly
            const expectedAuthMode = useDirectAuth0JWT ? 'auth0_direct' : 'supabase_jwt';
            const currentAuthMode = localStorage.getItem('auth_mode');

            // If switching FROM auth0_direct TO supabase_jwt, we need to create a Supabase session
            // because auth0_direct mode doesn't create one
            if (currentAuthMode === 'auth0_direct' && expectedAuthMode === 'supabase_jwt') {
              log('Switching from auth0_direct to supabase_jwt - need to create Supabase session');

              const { email, name, sub } = auth0State.user;
              const auth0OrgId = auth0State.user[`${namespace}/org_id`];
              ...
              try {
                ...
                if (sessionResponse.ok) {
                  ...
                  // Update localStorage with fresh data from middleware
                  if (sessionData.user) {
                  ...
                  if (sessionError) {
                    error('Failed to set Supabase session during mode switch:', sessionError);
                  } else {
                    log('✅ Supabase session created after mode switch');
                  }
                } else {
                  warn('Failed to create Supabase session during mode switch');
                }
              } catch (err) {
                error('Error creating Supabase session during mode switch:', err);
              }

              localStorage.setItem('auth_mode', expectedAuthMode);
            } else if (currentAuthMode !== expectedAuthMode) {
              log(`Correcting auth_mode: ${currentAuthMode} -> ${expectedAuthMode}`);
              localStorage.setItem('auth_mode', expectedAuthMode);
            }
            ...
            // Dispatch auth-ready event to notify other components (e.g., OrganizationContext)
            ...
            // Hybrid approach: Call middleware first to ensure user exists in DB,
            // then use direct Auth0 JWT for all subsequent API calls
            // ========================================
            if (useDirectAuth0JWT) {
              ...
              // This handles new user creation and syncs organization_id/role
              let userData: any = null;
              try {
                ...
                if (syncResponse.ok) {
                  ...
                  // Persist subscription info if available
                  try {
                    if (syncData.subscription) {
                      const sub = syncData.subscription;
                      if (sub.plan) localStorage.setItem('subscription_plan', sub.plan);
                      if (sub.status) localStorage.setItem('subscription_status', sub.status);
                      if (sub.trial_end) localStorage.setItem('subscription_trial_end', sub.trial_end);
                      if (sub.current_period_end) localStorage.setItem('subscription_period_end', sub.current_period_end);
                  ...
                  // AUTO-DETECT PLATFORM ADMIN: If user is in Platform Org but has no Auth0 roles,
                  // force re-login with organization to get roles (platform:super_admin, etc.)
                  ...
                  if (isPlatformOrg && hasNoAuth0Roles && notAlreadyReauthenticating) {
                    ...
                    window.location.href = `https://${auth0Domain}/authorize?` +
                      ...
                      `&prompt=none`; // Silent re-auth (no login screen if already authenticated)
                  ...
                  // Middleware failed, but user might still exist in DB
                  // Continue with direct JWT and let RLS handle fallback
                  warn('Middleware sync failed, continuing with direct JWT (RLS will use DB fallback)');
                }
              } catch (err) {
                // Network error or middleware down - continue with direct JWT
              ...
              // Prefer middleware data if available, fallback to Auth0 claims
            ...
            try {
              ...
              if (sessionData.user) {
                ...
                // Store subscription details if present
                try {
                  if (sessionData.subscription) {
                    const sub = sessionData.subscription;
                    if (sub.plan) localStorage.setItem('subscription_plan', sub.plan);
        ...
        // Production-grade graceful fallback: Try Supabase session first, then localStorage
        // This prevents rapid logouts on slow networks or browser extension delays
        ...
        try {
          ...
          if (session && session.user) {
            ...
            // Dispatch auth-ready event for cached session
            window.dispatchEvent(new CustomEvent('auth-ready'));
            return Promise.resolve();
          }
        } catch (supabaseErr) {
        ...
        if (userId && userEmail) {
          ...
          // Dispatch auth-ready event for cached localStorage session
          window.dispatchEvent(new CustomEvent('auth-ready'));
          return Promise.resolve();
        }
    ...
    getIdentity: async () => {
      ...
      if (!userId || !userEmail) {
        // Use warn instead of error - this is expected when user isn't logged in
