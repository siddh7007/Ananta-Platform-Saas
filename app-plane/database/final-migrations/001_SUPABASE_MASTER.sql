-- ============================================================================
-- ULTIMATE MASTER MIGRATION: Supabase Database (App-Plane)
-- ============================================================================
-- Version: 5.0.0
-- Created: 2025-12-09
-- Database: app-plane-supabase-db-dev (port 27432)
--
-- This consolidated migration includes ALL tables, columns, functions, and
-- seed data from migrations 001-100+ merged into a single idempotent script.
--
-- USE THIS FOR:
...
--
-- IMPORTANT: Run on EMPTY database. For existing databases, use incremental migrations.
-- ============================================================================
...
COMMENT ON TABLE organizations IS 'Multi-tenant organizations (companies/workspaces)';

-- ============================================================================
-- Users (Auth0/Keycloak Compatible - Multi-Org Support)
-- ============================================================================
...
COMMENT ON TABLE users IS 'User accounts linked to external auth providers. Users can be in MULTIPLE organizations.';

-- Legacy user_profiles for backwards compatibility
...
-- Central Component Catalog (For cross-org component data)
...
-- BOMs (Bill of Materials) - COMPLETE with all columns from migrations 025, 048
CREATE TABLE IF NOT EXISTS boms (
    ...
    -- Source tracking (migration 048)
    source TEXT,

    -- Priority & S3 audit fields (migration 025)
...
CREATE INDEX IF NOT EXISTS idx_boms_priority_status ON boms(priority DESC, enrichment_status, created_at);

COMMENT ON COLUMN boms.priority IS 'Enrichment priority: high (customer BOMs) or normal (staff BOMs). Affects processing order and batch size.';
COMMENT ON COLUMN boms.raw_file_s3_key IS 'S3/MinIO key for original uploaded file (audit/recovery)';
COMMENT ON COLUMN boms.parsed_file_s3_key IS 'S3/MinIO key for parsed snapshot JSON (audit/verification)';

-- Add FK from bom_uploads to boms
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'bom_uploads_bom_id_fkey'
    ) THEN
...
-- BOM Line Items (with component_id from migration 050)
CREATE TABLE IF NOT EXISTS bom_line_items (
    ...
    -- Component linking (migration 050)
    component_id UUID,
...
COMMENT ON COLUMN bom_line_items.component_id IS 'Foreign key to component_catalog.id in PostgreSQL Component Vault. Links this BOM line item to a high-quality enriched component.';
...
CREATE INDEX IF NOT EXISTS idx_cns_config_organization ON cns_enrichment_config(organization_id);
...
CREATE TABLE IF NOT EXISTS cns_cost_tracking (
    ...
    created_at TIMESTAMP DEFAULT NOW(),
    UNIQUE(organization_id, date, ai_provider, ai_operation)
);

CREATE INDEX IF NOT EXISTS idx_cns_cost_org_date ON cns_cost_tracking(organization_id, date);

-- CNS Bulk Uploads (S3/MinIO Storage)
...
-- Alerts (with SUPPLY_CHAIN from migration 068)
CREATE TABLE IF NOT EXISTS alerts (
    ...
    alert_type TEXT CHECK (alert_type IN ('LIFECYCLE', 'RISK', 'PRICE', 'AVAILABILITY', 'COMPLIANCE', 'PCN', 'SUPPLY_CHAIN')) NOT NULL,
    title TEXT NOT NULL,
    message TEXT,
    is_read BOOLEAN DEFAULT FALSE,
...
CREATE INDEX IF NOT EXISTS idx_alerts_supply_chain ON alerts(alert_type) WHERE alert_type = 'SUPPLY_CHAIN';
...
-- Component Watches (user subscriptions to component updates, with watch_supply_chain from migration 068)
...
-- Alert Preferences (with SUPPLY_CHAIN from migration 068)
...
-- Component Price History (from migration 068)
...
-- Get current user ID from JWT (multi-org compatible)
...
-- Get user role in a specific org
...
-- Current user organization helper (backwards compatible)
...
DROP TRIGGER IF EXISTS update_bom_line_items_updated_at ON bom_line_items;
CREATE TRIGGER update_bom_line_items_updated_at BEFORE UPDATE ON bom_line_items
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_cns_config_updated_at ON cns_enrichment_config;
CREATE TRIGGER update_cns_config_updated_at BEFORE UPDATE ON cns_enrichment_config
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_cns_bulk_uploads_updated_at ON cns_bulk_uploads;
CREATE TRIGGER update_cns_bulk_uploads_updated_at BEFORE UPDATE ON cns_bulk_uploads
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- Component search vector trigger
DROP TRIGGER IF EXISTS update_component_search ON components;
...
USING (
    ...
    AND (is_super_admin() OR id IN (SELECT get_user_organization_ids()))
);

CREATE POLICY "Users can view own org users" ON user_profiles FOR SELECT
    USING (organization_id IN (SELECT get_user_organization_ids()) OR is_super_admin());
...
CREATE POLICY "Users can insert boms in own org" ON boms FOR INSERT
    WITH CHECK (organization_id IN (SELECT get_user_organization_ids()) OR is_super_admin());

CREATE POLICY "Users can update boms in own org" ON boms FOR UPDATE
    USING (organization_id IN (SELECT get_user_organization_ids()) OR is_super_admin());
...
CREATE POLICY "Users can view own org enrichment events" ON enrichment_events
    FOR SELECT USING (tenant_id IN (SELECT get_user_organization_ids()) OR is_super_admin());
...
-- Price history
CREATE POLICY "Service role full access to price history" ON component_price_history
...
-- Seed: Platform Super Admin organization for CNS staff
...
-- Seed: Default Ananta Platform organization
INSERT INTO organizations (id, name, slug, subscription_status, plan_tier)
...
GRANT SELECT, INSERT, UPDATE ON boms TO anon, authenticated, service_role;
...
BEGIN
    ...
    RAISE NOTICE '';
    RAISE NOTICE '============================================================';
    RAISE NOTICE '  ULTIMATE MASTER MIGRATION V5.0 COMPLETE';
    RAISE NOTICE '  Supabase Database (App-Plane)';
    RAISE NOTICE '============================================================';
    RAISE NOTICE '  Total Tables: %', table_count;
    RAISE NOTICE '';
    RAISE NOTICE 'Includes ALL columns from migrations 001-100+:';
    RAISE NOTICE '  - boms.priority, raw_file_s3_key, parsed_file_s3_key (mig 025)';
    RAISE NOTICE '  - boms.source (mig 048)';
    RAISE NOTICE '  - bom_line_items.component_id (mig 050)';
    RAISE NOTICE '  - component_watches.watch_supply_chain (mig 068)';
    RAISE NOTICE '  - component_price_history table (mig 068)';
    RAISE NOTICE '  - Multi-org auth functions (mig 082)';
    RAISE NOTICE '';
    RAISE NOTICE 'Features Enabled:';
    RAISE NOTICE '  - Multi-org user management (Auth0/Keycloak)';
    RAISE NOTICE '  - Full-text search on components';
    RAISE NOTICE '  - Row Level Security (RLS)';
    RAISE NOTICE '  - Auto-update timestamps';
    RAISE NOTICE '  - Price history tracking';
    RAISE NOTICE '  - Supply chain alerts';
    RAISE NOTICE '============================================================';
END $$;
