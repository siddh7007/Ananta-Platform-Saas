                    ...
                    try:
                        insert_query = text("""
                            INSERT INTO enrichment_events (
                                event_id, event_type, routing_key, bom_id, organization_id,
        ...
        if component:
            ...
            if task.source == 'staff':
                ...
                try:
                    ...
                    supabase_db.commit()
                    logger.info(f"✅ Found in catalog, linked in Supabase: {task.mpn} (ID: {component_id})")
                finally:
                    try:
                        next(supabase_db_gen)
                    except StopIteration:
            ...
            if task.source == 'staff':
                ...
                if storage_location == 'database':
                    # Reference to Components V2 database
                    update_query = text("""
                        UPDATE bom_line_items
                        SET
                            component_id = :component_id,
                            enrichment_status = 'enriched',
                            component_storage = 'database'
                        WHERE id = :line_item_id
                    """)
                    supabase_db.execute(update_query, {
                        "component_id": component_id,
                    ...
                    # Reference to Redis temporary storage
                    update_query = text("""
                        UPDATE bom_line_items
                        SET
...
async def update_bom_progress(params: Dict[str, Any]) -> None:
    ...
    Args:
        params: {bom_id, progress, source (optional, default='customer')}
    """
    from app.models.dual_database import get_dual_database
    from sqlalchemy import text
    import json
    ...
    logger.info(f"Updating progress for BOM: {bom_id} (source={source}, status={status})")
    ...
    try:
        if source == 'staff':
            ...
            redis_storage.update_progress(
                ...
                ttl_hours=48  # Extended TTL during processing
            )

            # Update status if provided
            if status:
                redis_storage.set_status(status, ttl_hours=48)
                logger.info(f"✅ Status updated in Redis: {status}")

            logger.info(f"✅ Progress updated in Redis: {progress['percent_complete']:.1f}% complete")
        else:
            # Update Supabase for customer BOMs
            dual_db = get_dual_database()
            db = next(dual_db.get_session("supabase"))
            ...
            if status:
                ...
                logger.info(f"✅ Progress and status updated in Supabase: {progress['percent_complete']:.1f}% complete, status={status}")
            else:
                query = text("""
                    UPDATE boms
                    SET enrichment_progress = CAST(:progress AS jsonb)
                ...
                logger.info(f"✅ Progress updated in Supabase: {progress['percent_complete']:.1f}% complete")

            db.commit()

    except Exception as e:
        logger.error(f"Error updating progress (source={source}): {e}", exc_info=True)
        raise
    ...
    try:
        ...
        try:
            ...
            supabase_db.execute(insert_query, {
                ...
                "organization_id": str(event_record['organization_id']),
