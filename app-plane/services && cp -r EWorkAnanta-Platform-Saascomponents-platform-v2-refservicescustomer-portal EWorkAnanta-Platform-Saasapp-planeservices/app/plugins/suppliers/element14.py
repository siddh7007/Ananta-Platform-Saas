...
class Element14Plugin(SupplierPlugin):
    ...
    def search_by_mpn(
        ...
        # Build search term - combine manufacturer + MPN for better matching
        search_term = f'manuPartNum:{mpn}'
        if manufacturer:
            # Element14 uses vendor name filtering
            search_term = f'{manufacturer} {search_term}'
    ...
    def get_product_details(
        ...
        """
        # Get raw API response using search - combine manufacturer + MPN for better matching
        search_term = f'manuPartNum:{mpn}'
        if manufacturer:
            search_term = f'{manufacturer} {search_term}'
        ...
        try:
            ...
            # Lifecycle status - Element14 may have 'releaseStatusCode' or similar
            lifecycle = product.get('releaseStatusCode') or product.get('releaseStatus', '')

            # Image URL - Element14 uses 'image' object
            image_url = None
            image_obj = product.get('image')
            if image_obj:
                image_url = image_obj.get('vrntPath') or image_obj.get('baseName')

            # Availability - Element14 uses 'stock' object
            availability = 0
            stock_info = product.get('stock', {})
            if stock_info:
                availability = stock_info.get('level', 0)
                # Also check 'comInventory' for alternative availability info
                com_inventory = product.get('comInventory', {})
                if com_inventory and not availability:
                    availability = com_inventory.get('quantityAvailable', 0)
            ...
            # Lead time - Element14 may have 'stock.leadTime' or 'stock.leastLeadTime'
            lead_time_days = None
            if stock_info:
                lead_time_str = stock_info.get('leastLeadTime') or stock_info.get('leadTime', '')
                if lead_time_str:
                    try:
                        # Parse lead time (could be "6 weeks", "42 days", etc.)
                        import re
                        match = re.search(r'(\d+)', str(lead_time_str))
                        if match:
                            lead_time_val = int(match.group(1))
                            # Convert weeks to days if needed
                            if 'week' in str(lead_time_str).lower():
                                lead_time_days = lead_time_val * 7
                                logger.debug(f"Element14: Converted lead time {lead_time_val} weeks → {lead_time_days} days")
                            else:
                                lead_time_days = lead_time_val
                                logger.debug(f"Element14: Extracted lead time {lead_time_days} days from '{lead_time_str}'")
                    except Exception as e:
                        logger.debug(f"Element14: Failed to parse lead time '{lead_time_str}': {e}")
                        pass
            ...
            if attributes:
                for attr in attributes:
                    ...
                    if attr_name and attr_value:
                        ...
                        if attr_name in ['Package', 'Package / Case', 'Case', 'Case Style', 'Package Type', 'Case Type', 'Package Style']:
                            package = attr_value
            ...
            # Minimum Order Quantity - Element14 may have this in various fields
            min_order_qty = product.get('minimumOrderQuantity') or product.get('minOrderQty') or product.get('orderQuantity')
            if min_order_qty:
                parameters['minimum_order_quantity'] = str(min_order_qty)

            # Packaging type - Extract from attributes if available
            # Element14 may have 'Packaging' in attributes (e.g., "Reel", "Tube", "Bulk")
            packaging_type = None
            if attributes:
                for attr in attributes:
                    attr_label = attr.get('attributeLabel', '').lower()
                    if attr_label in ['packaging', 'packaging type', 'tape & reel']:
                        packaging_type = attr.get('attributeValue')
                        parameters['packaging'] = packaging_type
                        break

            # RoHS compliance - Element14 uses 'rohsStatusCode'
            rohs_status = product.get('rohsStatusCode', '')
            rohs_compliant = None
            if rohs_status:
                if rohs_status.lower() in ['compliant', 'rohs3 compliant']:
                    rohs_compliant = True
                elif rohs_status.lower() in ['non-compliant', 'not compliant']:
                    rohs_compliant = False

            # ECCN code - Element14 may have this in 'exportControlClassificationNumber'
            eccn_code = product.get('exportControlClassificationNumber') or product.get('eccnCode')

            # HTS code - Element14 may have this in compliance fields or product object
            hts_code = product.get('htsCode') or product.get('htsuscode') or product.get('harmonizedTariffCode')
            if hts_code:
                parameters['hts_code'] = hts_code

            # Package was already extracted in the attributes loop above (lines 291-294)
