...
export default function CustomerBOMs() {
  ...
  const post = async (url: string, body?: any, successMsg?: string) => {
    try {
      ...
      const adminHeaders = getAdminAuthHeaders() as Record<string, string> | undefined;
              ...
              <Grid item xs={12} md={3}>
                <AsyncAutocomplete
                  ...
                  loadOptions={async (query) => {
                    ...
                    try {
                      const res = await fetch(url.toString(), { headers: getAdminAuthHeaders() });
                      if (res.ok) {
                        const data = await res.json();
                    ...
                    <TableRow hover sx={{ '& > *': { borderBottom: isExpanded ? 'none' : undefined } }}>
                      ...
                      <TableCell padding="checkbox">
                        <Tooltip title={isExpanded ? 'Collapse' : 'View Line Items'}>
                          <IconButton size="small" onClick={() => toggleExpandRow(r.id)}>
                      ...
                      <TableCell align="right">
                        ...
                        {r.status !== 'enriching' && r.status !== 'paused' && (
                          <Tooltip title="Start enrichment"><span><IconButton size="small" onClick={()=>start(r.id)}><PlayArrowIcon fontSize="small" /></IconButton></span></Tooltip>
                        )}
                        {/* Show Resume only when paused */}
                        {r.status === 'paused' && (
                          <Tooltip title="Resume"><span><IconButton size="small" onClick={()=>resume(r.id)}><PlayArrowIcon fontSize="small" /></IconButton></span></Tooltip>
                        )}
                        {/* Show Pause and Stop only when enriching */}
                        {r.status === 'enriching' && (
                          <>
                            <Tooltip title="Pause"><span><IconButton size="small" onClick={()=>pause(r.id)}><PauseIcon fontSize="small" /></IconButton></span></Tooltip>
                            <Tooltip title="Stop"><span><IconButton size="small" onClick={()=>stop(r.id)}><StopIcon fontSize="small" /></IconButton></span></Tooltip>
                          </>
                        )}
                      </TableCell>
                    </TableRow>
                                  ...
                                  <TableBody>
                                    {bomLineItems.map((item) => (
                                      <TableRow
                                        key={item.id}
                                        sx={{
                                          bgcolor: item.enrichment_status === 'failed' ? 'error.50' :
                                                   item.enrichment_status === 'completed' ? 'success.50' : undefined,
