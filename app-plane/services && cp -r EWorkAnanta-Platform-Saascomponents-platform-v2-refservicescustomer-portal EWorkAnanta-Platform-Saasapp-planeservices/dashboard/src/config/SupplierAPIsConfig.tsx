...
const buildAdminHeaders = (extra?: Record<string, string>) => {
  ...
  if (!headersInit) {
    throw new Error(
      'Admin API token missing. Set VITE_CNS_ADMIN_TOKEN (or localStorage cns_admin_api_token) before using DigiKey OAuth actions.'
    );
  }
...
export const SupplierAPIsConfig: React.FC = () => {
  ...
  if (loading) {
    return (
      <Box p={3}>
        <Typography variant="h4" gutterBottom>Loading supplier configurations...</Typography>
        <LinearProgress />
      </Box>
    );
  }
  ...
  return (
    <Box p={3}>
      ...
      <Typography variant="body2" color="textSecondary" paragraph>
        Configure API keys and credentials for all supplier tiers (Distributors, Aggregators, OEMs)
      </Typography>
