/**
 * My Component Vault Filter Panel
 * CBP-P2-003: Filters for My Components (scoped to user's BOM line items)
 *
 * This filter panel is specifically for the My Components page, which queries
 * the user's bom_line_items instead of the global component_catalog.
 * The filters work within the selected workspace scope.
 */
...
interface MyComponentVaultFilterPanelProps {
  facets: SearchFacets;
...
export function MyComponentVaultFilterPanel({
  facets,
  filters,
  onChange,
  onClear,
  workspaceName,
}: MyComponentVaultFilterPanelProps) {
  const [manufacturerSearch, setManufacturerSearch] = useState('');
  const [categorySearch, setCategorySearch] = useState('');

  const activeFilterCount = Object.entries(filters).filter(([key, value]) => {
    // Exclude scope filters and sort from count (they're shown separately)
    if (['sortBy', 'sortOrder', 'workspaceId', 'projectId', 'bomId'].includes(key)) return false;
    ...
    <div className="p-4 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-lg">My Component Vault</h3>
        </div>
        {activeFilterCount > 0 && (
      ...
      {workspaceName && (
        <div className="text-xs text-muted-foreground bg-muted/50 rounded-md px-3 py-2 border">
          Filtering components within <span className="font-medium text-foreground">{workspaceName}</span>
        </div>
      )}
...
export default MyComponentVaultFilterPanel;
