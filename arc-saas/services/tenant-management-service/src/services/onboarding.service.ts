...
import {Address, Contact, Lead, Tenant, TenantOnboardDTO, SubscriptionDTO} from '../models';
...
export class OnboardingService {
  ...
  constructor(
    ...
    @service(ProvisioningRetryService)
  ...
  async addLead(lead: Omit<CreateLeadDTO, 'isValidated' | 'addressId' | 'id'>) {
    ...
    try {
      await this.auditLogger.logLeadCreated(newLead.id, lead.email);
    } catch {
  ...
  async onboard(dto: TenantOnboardDTO, lead?: Lead): Promise<Tenant> {
    ...
    try {
      ...
      // Determine contact email - prefer adminEmail from DTO, fallback to contact.email
      const contactEmail = dto.adminEmail ?? dto.contact?.email;

      // Generate default domains if not provided
      let domains = dto.domains;
      if (!domains || domains.length === 0) {
        // Default to key-based domain
        domains = [`${dto.key}.localhost`];
        // Also add the domain from contact/admin email if available
        if (contactEmail) {
      ...
      // Create contact - either from contact object or from adminEmail
      if (dto.contact?.email) {
        const contactHostName = dto.contact.email.split('@')[1];
        if (!domains.includes(contactHostName)) {
        ...
        // Create contact from adminEmail if contact object not provided
        await this.contactRepository.create(
          {
      ...
      // Log lead conversion if this was from a lead (non-blocking)
      if (lead?.id) {
        try {
          await this.auditLogger.logLeadConverted(
            lead.id,
            lead.email,
            res.id,
            res.name || 'unknown',
      ...
      // Auto-trigger provisioning workflow after tenant creation
      // The workflow handles: Keycloak realm creation, admin user creation, notifications
      // Create a basic subscription for provisioning (defined outside try for catch access)
      const subscription = {
        id: `sub-${res.id}`,
        planId: dto.planId || 'plan-basic',
        startDate: new Date().toISOString(),
        endDate: new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString(), // 1 year
      };

      try {
        this.logger.info(`Auto-starting provisioning workflow for tenant: ${res.id}`);
        await this.temporalProvisioningService.provisionTenant(res, subscription as SubscriptionDTO);
        this.logger.info(`Provisioning workflow started for tenant: ${res.id}`);
      } catch (provisionError) {
        // Queue for retry instead of just logging, preserve subscription context
        await this.retryService.queueForRetry(
          res.id,
          subscription.id,
          provisionError instanceof Error ? provisionError.message : 'Unknown error',
          subscription, // Pass original subscription context for retries
        );
        this.logger.warn(
          `Provisioning queued for retry for tenant ${res.id}: ${provisionError instanceof Error ? provisionError.message : 'Unknown error'}`,
        );
      }
