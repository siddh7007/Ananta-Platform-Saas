...
export class Tenant extends UserModifiableEntity {
  ...
  @property({
    type: 'string',
    description:
      'a short string used to identify a tenant. This is also used as the namespace and subdomain for this particular tenant. Must be 2-10 lowercase alphanumeric characters.',
    required: true,
    jsonSchema: {
  ...
  @property({
    ...
    description:
      'identity provider type for this tenant - keycloak, auth0, or cognito',
    default: 'keycloak',
    required: true,
    jsonSchema: {
      enum: ['keycloak', 'auth0', 'cognito'],
    },
  })
