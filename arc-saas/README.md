...
```bash
# 1. Start infrastructure (PostgreSQL, Redis, Keycloak, Temporal)
docker-compose up -d
docker-compose -f docker-compose.temporal.yml up -d

# 2. Install dependencies (uses Bun for speed)
cd services/tenant-management-service && bun install
cd ../temporal-worker-service && bun install
cd ../../apps/admin-app && bun install

# 3. Build services
cd services/tenant-management-service && bun run build
cd ../temporal-worker-service && bun run build

# 4. Run migrations
cd services/tenant-management-service && bun run migrate

# 5. Start services
cd services/tenant-management-service && bun run start &
cd services/temporal-worker-service && bun run start &
cd apps/admin-app && bun run dev &
```
...
### Tenancy Model
| Plan | Model | Infrastructure |
|------|-------|----------------|
| Free/Basic/Standard/Premium | Pooled | Shared Supabase + RLS |
| Enterprise | Silo | Dedicated PostgreSQL |
