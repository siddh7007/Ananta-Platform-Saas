...
export const BOMUploadWizard: React.FC<BOMUploadWizardProps> = ({
  ...
  const handleUploadFile = async () => {
    ...
    console.log('[BOMUploadWizard] 🚀 Starting upload:', {
      ...
      projectId: projectId,
      endpoint: `${CNS_API_URL}/boms/upload`
    });
    ...
    try {
      ...
      console.log('[BOMUploadWizard] 📤 Sending request to:', `${CNS_API_URL}/boms/upload`);
        ...
        <Box mb={3}>
          <Alert severity="info" icon={<CheckCircleIcon />}>
            <Typography variant="body2">
              <strong>File:</strong> {uploadResponse.filename} ({uploadResponse.file_type?.toUpperCase() || 'UNKNOWN'})
              <br />
