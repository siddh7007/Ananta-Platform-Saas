 ...
 * PRODUCTION MODE: Always uses real authentication (Mock with service_role for now, Keycloak later)
...
const getAuthProvider = (): AuthProvider => {
  ...
  const authProvider = import.meta.env.VITE_AUTH_PROVIDER?.toLowerCase() || 'supabase';
  ...
  if (authConfig.mode === 'production') {
    ...
    if (authProvider === 'keycloak') {
      if (authConfig.logAuthEvents) {
        console.log('✅ Using Keycloak authentication (recommended for ARC-SaaS)');
      }
  ...
  if (authProvider === 'keycloak') {
    if (authConfig.logAuthEvents) {
      console.log(
        ...
        'Dev bypass is disabled. Using Keycloak auth flow (recommended for ARC-SaaS).'
      );
    }
    return keycloakAuthProvider;
  }
 ...
 * See keycloakAuthProvider.ts for production authentication (future).
 */
