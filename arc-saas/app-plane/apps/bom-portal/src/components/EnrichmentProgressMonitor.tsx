/**
 * Enrichment Progress Monitor Component
 *
 * Real-time BOM enrichment progress display using Supabase Realtime.
...
import { useEnrichmentProgress, EnrichmentEvent, ComponentEvent } from '../hooks/useEnrichmentProgress';
...
export const EnrichmentProgressMonitor: React.FC<EnrichmentProgressMonitorProps> = ({
  ...
  const { state, componentEvents, isConnected, error, latestEvent } = useEnrichmentProgress({
    bomId,
    ...
    <Paper elevation={2} sx={{ p: 3, mb: 2 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
          <Box sx={{ flex: 1, mr: 2 }}>
            <LinearProgress
              variant="determinate"
              value={progress}
              color={enrichmentStatus.color === 'error' ? 'error' : 'primary'}
      ...
      {state && (
        <Box sx={{ display: 'flex', gap: 1, mb: 2, flexWrap: 'wrap' }}>
          <Chip
            label={`Total: ${state.total_items}`}
            variant="outlined"
            size="small"
          />
          <Chip
            label={`Enriched: ${state.enriched_items}`}
            color="success"
            variant="outlined"
            size="small"
          />
          <Chip
            label={`Pending: ${state.pending_items}`}
            color="info"
            variant="outlined"
            size="small"
          />
          {state.failed_items > 0 && (
            <Chip
              label={`Failed: ${state.failed_items}`}
              color="error"
              variant="outlined"
              size="small"
            />
          ...
          {state.not_found_items && state.not_found_items > 0 && (
            <Chip
              label={`Not Found: ${state.not_found_items}`}
              color="warning"
              variant="outlined"
              size="small"
            />
        ...
        <Box>
          ...
          >
            ...
            <IconButton size="small">
              {showComponentLog ? <ExpandLessIcon /> : <ExpandMoreIcon />}
            </IconButton>
          </Box>
          ...
          <Collapse in={showComponentLog}>
            ...
            >
              <List dense>
                {componentEvents.slice().reverse().map((component, index) => (
                  <ListItem
