...
export function ComparisonTray({
  ...
  const comparisonRows: Array<{
    ...
    { label: 'Quality Score', key: 'quality_score', format: (v) => `${v}%` },
    { label: 'Lifecycle', key: 'lifecycle_status' },
    { label: 'Unit Price', key: 'unit_price', format: (v) => `$${v.toFixed(2)}` },
    { label: 'Stock', key: 'stock_quantity', format: (v) => v.toLocaleString() },
    { label: 'Lead Time', key: 'lead_time_days', format: (v) => `${v} days` },
    { label: 'RoHS', key: 'rohs_compliant', format: (v) => (v ? 'Yes' : 'No') },
  ];
      ...
      >
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            ...
            <Chip
              label={`${components.length}/${maxComponents}`}
              size="small"
