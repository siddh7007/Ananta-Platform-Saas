  ...
  useEffect(() => {
    ...
    const fetchInitialStatus = async () => {
      try {
        ...
        // Get latest enrichment event for this BOM
        ...
        if (events && events.length > 0) {
          ...
          console.log('[Enrichment SSE] Initial status loaded:', latestEvent.state?.status);

          // Set initial state from database
          if (latestEvent.state) {
            setState(latestEvent.state);
            setLatestEvent(latestEvent);
            setEvents([latestEvent]);
          }
        } else {
          console.log('[Enrichment SSE] No existing enrichment events found');
        }
      } catch (err) {
