    ...
    try:
        with get_supabase_session() as session:
            ...
            existing = session.execute(
                text("""
                    SELECT u.id, u.organization_id, o.auth0_org_id, om.role,
            ...
            if existing:
                ...
                return ProvisionUserResponse(
                    success=True,
                    auth0_org_id=existing.auth0_org_id or generate_synthetic_org_id(auth0_user_id),
            ...
            org_result = session.execute(
                text("""
                    INSERT INTO organizations (name, slug, auth0_org_id, org_type, max_users, trial_ends_at, created_at)
                    VALUES (:name, :slug, :auth0_org_id, :org_type, :max_users, :trial_ends_at, NOW())
                    RETURNING id, name
                """),
                {
                    "name": org_name,
    ...
    try:
        with get_supabase_session() as session:
            result = session.execute(
                text("""
                    SELECT u.id, o.auth0_org_id, om.role
                    FROM users u
                    LEFT JOIN organizations o ON o.id = u.organization_id
                    LEFT JOIN organization_memberships om ON om.user_id = u.id AND om.organization_id = u.organization_id
            ...
            org_roles_rows = session.execute(
                text("""
                    SELECT o.auth0_org_id, om.role
                    FROM organization_memberships om
                    JOIN organizations o ON o.id = om.organization_id
                    WHERE om.user_id = :user_id AND o.auth0_org_id IS NOT NULL
                """),
                {"user_id": result.id}
            ).fetchall()
            org_roles = {
                row.auth0_org_id: row.role or "member"
            ...
            return UserLookupResponse(
                exists=True,
                auth0_org_id=result.auth0_org_id,
                roles=[customer_role],  # Include customer org role for JWT sync
                org_roles=org_roles,
            )
