...
async def get_account_status(request: Request):
    ...
    try:
        with get_supabase_session() as db:
            ...
            days_remaining = None
            if result.deletion_scheduled_at:
            ...
            return AccountStatusResponse(
                ...
                org_type=result.org_type or "individual",
                is_active=not result.is_suspended,
                is_suspended=result.is_suspended or False,
                suspended_reason=result.suspended_reason,
                is_pending_deletion=result.deletion_scheduled_at is not None,
                deletion_scheduled_at=result.deletion_scheduled_at,
                deletion_days_remaining=days_remaining,
                member_count=member_count,
                created_at=result.created_at,
                can_be_deleted=can_be_deleted,
                plan_tier="free",  # TODO: Get from subscription data
                subscription_status="active" if not result.is_suspended else "suspended",
            )
