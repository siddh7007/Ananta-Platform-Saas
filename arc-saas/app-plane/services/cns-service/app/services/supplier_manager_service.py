...
def initialize_supplier_manager() -> SupplierPluginManager:
    ...
    """
    logger.info("🔌 Initializing supplier plugin manager...")
    ...
    if settings.mouser_enabled:
        ...
        if settings.mouser_api_key:
            logger.info("  ✅ Mouser: enabled")
        else:
            logger.warning("  ⚠️  Mouser: disabled (no API key)")
    else:
        logger.info("  ⏭️  Mouser: disabled by config")
    ...
    if settings.digikey_enabled:
        ...
        if has_credentials and has_access_token:
            logger.info("  ✅ DigiKey: enabled")
        elif has_credentials:
            logger.warning("  ⚠️  DigiKey: configured but missing OAuth2 access token")
            logger.warning("      Set DIGIKEY_ACCESS_TOKEN to enable DigiKey API")
            supplier_config['digikey']['enabled'] = False
        else:
            logger.info("  ⏭️  DigiKey: disabled (no credentials)")
    else:
        logger.info("  ⏭️  DigiKey: disabled by config")
        supplier_config['digikey'] = {'enabled': False}
    ...
    if settings.element14_enabled:
        ...
        if settings.element14_api_key:
            logger.info(f"  ✅ Element14: enabled (store={settings.element14_store})")
        else:
            logger.warning("  ⚠️  Element14: disabled (no API key)")
    else:
        logger.info("  ⏭️  Element14: disabled by config")
    ...
    try:
        ...
        if available_suppliers:
            logger.info(f"✅ Supplier plugin manager initialized with {len(available_suppliers)} supplier(s): {', '.join(available_suppliers)}")
        else:
            logger.warning("⚠️  No suppliers are available - enrichment will use fallback/mock data")

        return manager

    except Exception as e:
        logger.error(f"❌ Failed to initialize supplier plugin manager: {e}")
